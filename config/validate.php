...
////////////////////				////////////////////				////////////////////
	...
	if(strlen(trim($_POST['Nombre'])) == 0){
		...
		$errors [] = "NOMBRE: <font color='#F1BD2D'>MAS DE DOS CARACTERES</font>";
	}elseif(!preg_match('/^[^0-9@´`\'áéíóú#$&%<>:"·\(\)=¿?!¡\[\]\{\};,:\.\*]+$/',$_POST['Nombre'])){
		$errors [] = "NOMBRE: <font color='#F1BD2D'>TEXOT SIN ACENTOS</font>";
	}
		
	/* VALIDAMOS EL CAMPO APELLIDOS. */
	if(strlen(trim($_POST['Apellidos'])) == 0){
		$errors [] = "APELLIDOS: <font color='#F1BD2D'>CAMPO OBLIGATORIO</font>";
	}elseif(strlen(trim($_POST['Apellidos'])) < 4){
		$errors [] = "APELLIDOS: <font color='#F1BD2D'>MAS DE TRES CARACTERES</font>";
	}elseif(!preg_match('/^[^0-9@´`\'áéíóú#$&%<>:"·\(\)=¿?!¡\[\]\{\};,:\.\*]+$/',$_POST['Apellidos'])){
		$errors [] = "APELLIDOS: <font color='#F1BD2D'>TEXTO SIN ACENTOS</font>";
	}
...
////////////////////				////////////////////				////////////////////
	...
	if(isset($_POST['id']) == @$rowdni['id']){
		...
		$errors [] = "NUMERO DNI/NIF: <font color='#F1BD2D'>YA EXISTE</font>";
	}
		
	if($_POST['doc'] == 'DNI'){
		if(strlen(trim($_POST['dni'])) == 0){
			$errors [] = "NUMERO DNI/NIF: <font color='#F1BD2D'>CAMPO OBLIGATORIO</font>";
		}elseif(!preg_match('/^[\d]+$/',$_POST['dni'])){
			$errors [] = "NUMERO DNI/NIF: <font color='#F1BD2D'>SOLO NUMEROS</font>";
		}elseif(strlen(trim($_POST['dni'])) < 8){
			$errors [] = "NUMERO DNI/NIF: <font color='#F1BD2D'>MAS DE 7 CARACTERES</font>";
			}
	}
...
////////////////////				////////////////////				////////////////////
	...
	/* CONDICIONAL PARA TODOS LOS NIF CON NUMERO DE CONTROL */
		...
		if(!preg_match('/^[\d]+$/',$_POST['ldni'])){
			$errors [] = "NUMERO CONTROL NIF ESPECIAL: <font color='#F1BD2D'>SOLO NUMEROS</font>";
		}else{
...
////////////////////				////////////////////				////////////////////
	...
	if(strlen(trim($_POST['Nivel'])) == 0){
		$errors [] = "NIVEL: <font color='#F1BD2D'>CAMPO OBLIGATORIO</font>";
		}
	...
	if(strlen(trim($_POST['Email'])) == 0){
		$errors [] = "MAIL: <font color='#F1BD2D'>CAMPO OBLIGATORIOfont>";
	}elseif(strlen(trim($_POST['Email'])) < 5 ){
		$errors [] = "MAIL: <font color='#F1BD2D'>MAS DE CINCO CARACTERES</font>";
		...
		$errors [] = "MAIL: <font color='#F1BD2D'>DIRECCION NO VALIDA</font>";
	}
		
/* if(trim($_POST['id'] == $rowd['id'])&&(!strcasecmp($_POST['Email'] , $rowd['Email']))){}
			elseif(!strcasecmp($_POST['Email'] , $rowd['Email'])){
				$errors [] = "MAIL: <font color='#F1BD2D'>NO PERMITIDO</font>";
				}	
...
////////////////////				////////////////////				////////////////////
	...
	if(isset($_POST['id']) == @$rowus['id']){
		...
		$errors [] = "NICK: <font color='#F1BD2D'>YA EXISTE</font>";
	}

	if(strlen(trim($_POST['Usuario'])) == 0){
		$errors [] = "NICK: <font color='#F1BD2D'>CAMPO OBLIGATORIO</font>";
	}elseif(strlen(trim($_POST['Usuario'])) < 3){
		$errors [] = "NICK: <font color='#F1BD2D'>MAS DE TRES CARACTERES</font>";
	}elseif(!preg_match('/^\b[^@#$%&<>\?\[\]\{\}\+\s]+$/',$_POST['Usuario'])){
		$errors [] = "NICK: <font color='#F1BD2D'>SIN CARACTERES ESPECIALES</font>";
	}elseif(trim($_POST['Usuario'] != $_POST['Usuario2'])){
		$errors [] = "NICK: <font color='#F1BD2D'>NO SON IGUALES</font>";
	}
		
/*	if(trim($_POST['id'] == $rowd['id'])&&(!strcasecmp($_POST['Usuario'] , $rowd['Usuario']))){}
			elseif(!strcasecmp($_POST['Usuario'] , $rowd['Usuario'])){
				$errors [] = "NICK: <font color='#F1BD2D'>NICK NO PERMITIDO</font>";
				}
...
////////////////////				////////////////////				////////////////////
	...
	if(strlen(trim($_POST['Password'])) == 0){
		$errors [] = "PASSWORD: <font color='#F1BD2D'>CAMPO OBLIGATORIO</font>";
	}elseif(strlen(trim($_POST['Password'])) < 3){
		$errors [] = "PASSWORD: <font color='#F1BD2D'>MAS DE TRES CARACTERES</font>";
	}elseif(!preg_match('/^\b[^@#$%&<>\?\[\]\{\}\+\s]+$/',$_POST['Password'])){
		$errors [] = "PASSWORD: <font color='#F1BD2D'>SIN CARACTERES ESPECIALES</font>";
	}elseif(trim($_POST['Password'] != $_POST['Password2'])){
		$errors [] = "PASSWORD: <font color='#F1BD2D'>NO SON IGUALES</font>";
	}

	/* Validamos el campo Dirección. */
	if(strlen(trim($_POST['Direccion'])) == 0){
		$errors [] = "DIRECCION: <font color='#F1BD2D'>CAMPO OBLIGATORIO</font>";
	}elseif(!preg_match('/^\b[^@#$%&<>\?\[\]\{\}\+]+$/',$_POST['Direccion'])){
		$errors [] = "DIRECCION: <font color='#F1BD2D'>SIN CARACTERES ESPECIALES</font>";
	}
...
////////////////////				////////////////////				////////////////////
	...
	if(isset($_POST['id']) == @$rowtlf1['id']){
		...
		$errors [] = "TELEFONO 1: <font color='#F1BD2D'>YA EXISTE</font>";
	}

	if(strlen(trim($_POST['Tlf1'])) == 0){
		$errors [] = "TELEFONO 1: <font color='#F1BD2D'>CAMPO OBLIGATORIO</font>";
	}elseif((trim($_POST['Tlf1'])) == (trim($_POST['Tlf2']))){
					$errors [] = "TELEFONO 1 y 2: <font color='#F1BD2D'>SON IGUALES</font>";
	}elseif(!preg_match('/^[\d]+$/',$_POST['Tlf1'])){
		$errors [] = "TELEFONO 1: <font color='#F1BD2D'>SOLO NUMEROS</font>";
	}elseif(strlen(trim($_POST['Tlf1'])) < 9){
		$errors [] = "TELEFONO 1: <font color='#F1BD2D'>NUEVE NUMEROS</font>";
	}
...
////////////////////				////////////////////				////////////////////
	...
	if(strlen(trim($_POST['Tlf2'])) > 0){
		...
		if(isset($_POST['id']) == @$rowtlf2['id']){
			...
			$errors [] = "TELEFONO 2: <font color='#F1BD2D'>YA EXISTE</font>";
		}elseif(!preg_match('/^[\d]+$/',$_POST['Tlf2'])){
			$errors [] = "TELEFONO 2: <font color='#F1BD2D'>SOLO NUMEROS</font>";
		}elseif(strlen(trim($_POST['Tlf2'])) < 9){
			$errors [] = "TELEFONO 2: <font color='#F1BD2D'>9 NUMEROS</font>";
		}
	}

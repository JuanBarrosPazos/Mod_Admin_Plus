...
print("<font color='#FF0000'>293 Se ha producido un error: </font></br>".mysqli_error($db)."</br>");
		...
		/* PROCEDEMOS A BORRAR LAS TABLAS DEL USUARIO */
			...
			if(mysqli_query($db, $sqlt1)){
					...
					global $tx1; 		$tx1 = "\t* HA BORRADO LA TABLA ".$fila[0]."\n";
					global $deletet2; 	$deletet2 = $deletet2.$tx1;
				} else {
					global $tx1; 		$tx1 = "\t* ".mysqli_error($db)."\n";
					print ("<font color='#FF0000'>*** </font></br> ".mysqli_error($db).".</br>");
					global $deletet2; 	$deletet2 = $tx1;
							} 
		...
		global $deletet2;	$deletet1 = $dd1.$dd2."\n";
		global $deletet;	$deletet = $deletet1.$deletet2;
...
function deletedir(){
	...
	// BORRA EL CONTENIDO DEL DIRECTORIO DEL USUARIO, UNA VEZ LIMPIOS LOS SUBDIRECTORIOS
	...
	if(file_exists($carpeta0)){  $dir0 = $carpeta0."/";
								...
								rmdir (	$carpeta0);
								global $dd0;	$dd0 = "\t- BORRADA: ".$carpeta0."/ \n";
								} else {print("");}
	...
	print("<table style=\"margin-top:2px\">
				<tr>
					<th colspan=3 class='BorderInf'>
						<font color='#FF0000'>SE BORRARÁN Y NO SE PODRÁN VOLVER A RECUPERAR</font>
					</th>
				</tr>
				<tr>
